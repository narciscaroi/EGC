...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int spotLightLocation = glGetUniformLocation(shader->program, "modeSpotLightt");
	glUniform1i(spotLightLocation, modeSpotLight);

	int angle_location = glGetUniformLocation(shader->program, "cutOffAnglee");
	glUniform1f(angle_location, cutOffAngle);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_R)) {
			cutOffAngle += RADIANS(5) * deltaTime * 100;
			cout << "cutOffAngle: " << cutOffAngle << endl;
		}
...
void Laborator8::OnKeyPress(int key, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_F)) {
		cout << modeSpotLight << endl;
		modeSpotLight = (modeSpotLight + 1) % 2;
	}
}
