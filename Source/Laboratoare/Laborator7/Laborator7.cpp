...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	GLint locLightPosition = glGetUniformLocation(shader->program, "light_position");
	...
	// diffuse light
	locMaterial = glGetUniformLocation(shader->program, "material_kd");
	glUniform1f(locMaterial, materialKd);

	// specular light
	locMaterial = glGetUniformLocation(shader->program, "material_ks");
	glUniform1f(locMaterial, materialKs);
