...
void Laborator9::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-1.6f, 1.7f, 0.0f));
		modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));
		RenderSimpleMesh(meshes["square"], shaders["ShaderLab9"], modelMatrix, mapTextures["grass"], mapTextures["crate"], 1);
	}
}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2, int textureType)
{
	...
	int locTextureType = glGetUniformLocation(shader->program, "textureType");
	glUniform1i(locTextureType, textureType);

	if (strcmp(mesh->GetMeshID(), "sphere") == 0) {
		int locTimeElapsed = glGetUniformLocation(shader->program, "timeElapsed");
		glUniform1f(locTimeElapsed, timeElapsed);
	} 
	else {
		int locTimeElapsed = glGetUniformLocation(shader->program, "timeElapsed");
		glUniform1f(locTimeElapsed, 0);
	}
